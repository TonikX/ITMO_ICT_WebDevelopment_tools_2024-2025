...
def get_exchanges(session: Session = Depends(get_session)):
    exchanges = session.exec(
        select(Exchange)
        .options(
            joinedload(Exchange.offer)
            .joinedload(Offer.sender),
            joinedload(Exchange.offer)
            .joinedload(Offer.receiver),
            joinedload(Exchange.offer)
            .joinedload(Offer.sender_book).joinedload(UserBook.book),
            joinedload(Exchange.offer)
            .joinedload(Offer.receiver_book).joinedload(UserBook.book),
        )
    ).all()
    return exchanges
...
def get_exchange(exchange_id: int, session: Session = Depends(get_session)):
    statement = (
        ...
        .options(
            selectinload(Exchange.offer)
            .selectinload(Offer.sender),
            selectinload(Exchange.offer)
            .selectinload(Offer.receiver),
        )
    )
    exchange = session.exec(statement).first()
    if not exchange:
...
def create_exchange(exchange_create: ExchangeCreate, session: Session = Depends(get_session)):
    ...
    new_exchange.exchange_date = datetime.utcnow().isoformat()
...
def update_exchange(exchange_id: int, exchange_update: ExchangeCreate, session: Session = Depends(get_session)):
    ...
    # Обновим дату обмена, если нужно (по желанию)
    exchange.exchange_date = datetime.utcnow().isoformat()
