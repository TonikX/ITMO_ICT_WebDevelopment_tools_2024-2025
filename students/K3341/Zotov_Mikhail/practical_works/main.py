...
from Pr1_1.models import *
from Pr1_2.connection import *
...
@app.get("/warrior/{warrior_id}", response_model=WarriorView)
...
def warrior_delete(skill_id: int, session=Depends(get_session)):
    skill = session.get(Skill, skill_id)
    if not skill:
