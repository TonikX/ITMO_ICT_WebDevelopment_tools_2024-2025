...
from models.models import User, Category, Transaction, TransactionType
import os
...
def parse_and_save(url: str, user_id: int, category_name: str):
    try:
        ...
        with Session(engine) as session:
            ...
            session.commit()
            print(f"[OK] Сохранил данные из {url}")

    except Exception as e:
        print(f"[ERROR] Ошибка при парсинге {url}: {e}")

def run_parser(urls_with_categories: list[tuple[str, str]], user_id: int):
    threads = []
