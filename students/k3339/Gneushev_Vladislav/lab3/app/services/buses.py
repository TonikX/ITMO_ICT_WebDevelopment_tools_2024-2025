...
class BusService:
    ...
    async def add_bus_types(self, bus_types: list[BusType]) -> None:
        for bus_type in bus_types:
            await self.add_bus_type(bus_type)
    ...
    async def update_bus(
        ...
        if state_number is not None:
            bus.state_number = state_number

        return await self.bus_repo.update_bus(bus)
