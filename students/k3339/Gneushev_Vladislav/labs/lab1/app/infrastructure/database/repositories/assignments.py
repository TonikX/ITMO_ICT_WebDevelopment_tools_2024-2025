...
class AssignmentRepository(BaseRepository):
    async def get_assignments(
        ...
        if driver_id is not None:
            q = q.where(DriverAssignmentDB.driver_id == driver_id)
        if bus_id is not None:
            q = q.where(DriverAssignmentDB.bus_id == bus_id)
        if day_of_week is not None:
            q = q.where(DriverAssignmentDB.day_of_week == day_of_week)
