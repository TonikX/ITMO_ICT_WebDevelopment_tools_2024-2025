...
from app.presentation.api.routes.buses.schemas import GetBusSchema, AddBusSchema, GetBusTypeSchema, AddBusTypeSchema, UpdateBusSchema, PutBusSchema
...
async def update_bus(
    bus_id: int,
    bus_update: UpdateBusSchema,
    bus_service: FromDishka[BusService],
):
    bus = await bus_service.update_bus(
        bus_id=bus_id,
        state_number=bus_update.state_number,
        bus_type_id=bus_update.bus_type_id
    )
    return GetBusSchema.model_validate(bus, from_attributes=True)
...
@router.put(
    "/{bus_id}",
    summary="Полностью обновить автобус",
    response_model=GetBusSchema,
)
@only_admin
async def put_bus(
    bus_id: int,
    bus_update: PutBusSchema,
    bus_service: FromDishka[BusService],
):
    bus = await bus_service.put_bus(
        bus_id=bus_id,
        state_number=bus_update.state_number,
        bus_type_id=bus_update.bus_type_id
    )
