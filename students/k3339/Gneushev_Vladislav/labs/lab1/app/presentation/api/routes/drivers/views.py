...
from app.presentation.api.routes.drivers.schemas import GetDriverSchema, AddDriverSchema, GetDriverClassSchema, \
    AddDriverClassSchema, GetWorkScheduleSchema, DriverClassesCountSchema, ClassDriversCountSchema, UpdateDriverSchema, \
    PutDriverSchema
...
async def update_driver(
    driver_id: int,
    driver_update: UpdateDriverSchema,
    driver_service: FromDishka[DriverService],
):
    driver = await driver_service.update_driver(
        driver_id=driver_id,
        first_name=driver_update.first_name,
        last_name=driver_update.last_name,
        passport_info=driver_update.passport_info,
        driver_class_name=driver_update.driver_class_name,
        work_experience_months=driver_update.work_experience_months
    )
    return GetDriverSchema(
        id=driver.id,
        first_name=driver.first_name,
...
@router.put(
    "/{driver_id}",
    summary="Полностью обновить водителя",
    response_model=GetDriverSchema,
)
@only_admin
async def put_driver(
    driver_id: int,
    driver_update: PutDriverSchema,
    driver_service: FromDishka[DriverService],
):
    driver = await driver_service.put_driver(
        driver_id=driver_id,
        first_name=driver_update.first_name,
        last_name=driver_update.last_name,
        passport_info=driver_update.passport_info,
