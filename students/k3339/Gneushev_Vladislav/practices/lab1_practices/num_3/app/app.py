...
class SkillLinkRequest(SQLModel):
    skill_id: int
    level: Optional[int] = None


@app.post("/warrior/{warrior_id}/add_skill")
def add_skill_to_warrior(warrior_id: int, skill_link: SkillLinkRequest, session=Depends(get_session)):
    warrior = session.get(Warrior, warrior_id)
    if not warrior:
        raise HTTPException(status_code=404, detail="Warrior not found")

    skill = session.get(Skill, skill_link.skill_id)
    if not skill:
        raise HTTPException(status_code=404, detail="Skill not found")
    ...
    link = session.exec(
        select(SkillWarriorLink).where(
            ...
            SkillWarriorLink.skill_id == skill_link.skill_id
        )
    ).first()

    if link:
        link.level = skill_link.level
        session.add(link)
        session.commit()
        return {"message": "Skill level updated for warrior"}

    link = SkillWarriorLink(
...
def get_warrior_skills(warrior_id: int, session=Depends(get_session)) -> List[dict]:
    warrior = session.get(Warrior, warrior_id)
    if not warrior:
        raise HTTPException(status_code=404, detail="Warrior not found")

    links = session.exec(
        select(SkillWarriorLink, Skill).join(Skill).where(
            SkillWarriorLink.warrior_id == warrior_id
        )
    ).all()
...
def get_skill_warriors(skill_id: int, session=Depends(get_session)) -> List[dict]:
    skill = session.get(Skill, skill_id)
    if not skill:
        raise HTTPException(status_code=404, detail="Skill not found")

    links = session.exec(
        select(SkillWarriorLink, Warrior).join(Warrior).where(
            SkillWarriorLink.skill_id == skill_id
        )
    ).all()

    result = []
    for link, warrior in links:
        warrior_data = warrior.model_dump()
        warrior_data["skill_level"] = link.level
        result.append(warrior_data)
