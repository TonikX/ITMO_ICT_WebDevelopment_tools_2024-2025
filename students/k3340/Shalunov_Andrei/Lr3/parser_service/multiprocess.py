...
def parse_and_save(chunk: list[str]) -> int:
    filler = SyncDBFiller(dsn=os.getenv("DATABASE_URL"))
    saved = 0
    ...
    for url in chunk:
        try:
            ...
            if filler.add_book(details, source="process"):
                saved += 1
        except:
            continue
    filler.disconnect()
...
def main():
    ...
    print(f"[multiprocessing] saved {sum(results)} books in {time.time()-t0:.2f}s")
