...
def get_sprint(sprint_id: int, session: DatabaseSession, response: Response):
    stmt = select(SprintModel).options(selectinload(SprintModel.tasks)).where(SprintModel.id == sprint_id)
    try:
    ...
    return SprintDataResponse(status=200, data=SprintWithTasksResponse.model_validate(sprint_model))
...
def add_sprint(sprint_body: SprintBodySchema, session: DatabaseSession) -> SprintDataResponse:
    ...
    session.refresh(sprint_model, ['tasks'])
...
def delete_sprint(sprint_id: int, session: DatabaseSession, response: Response) -> MessageResponse:
    ...
    try:
        ...
        response.status_code = 404
        return MessageResponse(status=404, message="Sprint not found")
