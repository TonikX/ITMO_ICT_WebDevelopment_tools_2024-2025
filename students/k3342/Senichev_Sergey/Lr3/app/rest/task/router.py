...
def add_task(task_body: TaskBodySchema, session: DatabaseSession, response: Response):
    # Создаем задачу простым способом
    ...
    # Явно устанавливаем sprint в None для избежания lazy loading
    task_model.sprint = None
    if task_model.sprint_id:
        # Загружаем sprint если есть sprint_id
        sprint = session.get(SprintModel, task_model.sprint_id)
        task_model.sprint = sprint
