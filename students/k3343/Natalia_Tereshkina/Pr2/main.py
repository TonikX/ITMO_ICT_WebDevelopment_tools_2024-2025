...
@app.get("/user/{user_id}/budgets", response_model=List[Budget])
def get_user_budgets(user_id: int, session: Session = Depends(get_session)):
    budgets = session.exec(select(Budget).where(Budget.user_id == user_id)).all()
    return budgets

@app.post("/goal", response_model=Goal)
def create_goal(goal: GoalBase, session: Session = Depends(get_session)):
...
@app.get("/user/{user_id}/notifications", response_model=List[Notification])
