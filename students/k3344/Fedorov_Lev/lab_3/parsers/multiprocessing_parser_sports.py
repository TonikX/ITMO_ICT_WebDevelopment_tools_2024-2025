        ...
        if data_type == 'teams':
            ...
            if teams:
                ...
                try:
                    ...
                    conn.commit()
                    print(f"Process {process_id}: Saved {len(unique_teams)} teams from {url}")

                except Exception as e:
                    conn.rollback()
                    print(f"Process {process_id}: DB error: {e}")
                finally:
                    cursor.close()
                    conn.close()
            else:
                print(f"Process {process_id}: No teams found on {url}")
...
def multiprocessing_parser(url_list, num_processes=None):
    ...
    print(f"Запуск парсинга с использованием {num_processes} процессов")
