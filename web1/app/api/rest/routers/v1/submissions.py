...
@router.get("/get_all", response_model=list[Submission])
def get_all(session=Depends(get_session)):
    return submission_controller.get_all_submissions(session)

@router.get("/get_id/{submission_id}", response_model=SubmissionFull)
...
@router.post("/create_sub", response_model=SubmissionFull)
...
@router.delete("/delete_sub/{submission_id}")
