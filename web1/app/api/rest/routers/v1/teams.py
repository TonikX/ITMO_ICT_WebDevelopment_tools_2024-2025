...
@router.post("/add-member_to_team/{team_id}/{user_id}", response_model=TeamFull)
def add_team_member(
    team_id: int,
    user_id: int,
...
@router.delete("/delete-member_from_team/{team_id}/{user_id}", response_model=TeamFull)
...
@router.patch("/update_team/{team_id}", response_model=TeamFull)
def team_update(
    team_id: int,
    updates: TeamUpdate,
    auth_user_id: int = Depends(auth_handler.get_user),
